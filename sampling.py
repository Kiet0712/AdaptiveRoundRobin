...
def sampling_process(
    ...
    burst_time_array = np.random.normal(loc=mean_burst_time,scale=std_burst_time,size=num_process)
    arrival_time_array = np.random.normal(loc=mean_arrival_time,scale=std_arrival_time,size=num_process)
    result_sampling = []
